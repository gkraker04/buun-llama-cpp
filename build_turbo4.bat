...
cmake .. -G Ninja -DGGML_CUDA=ON -DGGML_CUDA_FA=ON -DGGML_CUDA_FA_ALL_QUANTS=ON -DGGML_NATIVE=ON -DCMAKE_BUILD_TYPE=Release -DCMAKE_CUDA_ARCHITECTURES=86-real -DBUILD_SHARED_LIBS=OFF -DLLAMA_BUILD_EXAMPLES=OFF -DCMAKE_CUDA_COMPILER="%CMAKE_CUDA_COMPILER%" -DLLAMA_BUILD_TESTS=OFF -DCMAKE_CUDA_FLAGS="-diag-suppress=177,550" >>%LOGFILE% 2>&1
if {%errorlevel%} neq {0} exit /b 1
