...
// ---------------------------------------------------------------------------
// in-tool EAR (optimization #1): compute EAR + KL_topk inside the tool per
// step, against a fixed F16 reference logits file.  Mirrors the Python path
// (ear_aggregate.analyze) exactly: decode u16 -> logp, re-logsoftmax, top-K
// under the REFERENCE p (constant across steps -> precomputed once), gather.
// ---------------------------------------------------------------------------

struct ear_ctx {
    bool enabled = false;
    int n_vocab   = 0;
    int n_pos_total = 0;              // n_chunk * n_pos_per_chunk
    int n_pos_per_chunk = 0;          // n_ctx - 1 - n_ctx/2
    int n_chunk = 0;
    std::vector<int32_t> topk;        // n_pos_total * 10
    std::vector<float> ref_bp_topk;   // n_pos_total * 10 (log-softmax ref at topk)
    std::vector<float> ref_p_topk;    // n_pos_total * 10 (exp)
    std::vector<float> scratch;       // n_vocab decode scratch
    // per-step accumulation
    int    n_seen = 0;
    double ear_sum = 0.0;
    double kl_sum  = 0.0;
    std::ofstream out;                // --ear-out JSON lines
};

static bool ear_load_ref(ear_ctx & ear, const std::string & path) {
    std::ifstream f(path, std::ios::binary);
    if (!f.is_open()) {
        LOG_ERR("ear: failed to open ref %s", path.c_str());
        return false;
    }
    char magic[9] = {0};
    f.read(magic, 8);
    if (strcmp(magic, "_logits_") != 0) {
        LOG_ERR("ear: bad magic in %s", path.c_str());
        return false;
    }
    uint32_t n_ctx;
    int32_t n_vocab, n_chunk;
    f.read((char *) &n_ctx, 4);
    f.read((char *) &n_vocab, 4);
    f.read((char *) &n_chunk, 4);
    f.seekg(n_chunk * n_ctx * 4, std::ios::cur);   // tokens
    const int nv = 2*((n_vocab + 1)/2) + 4;
    const int n_pos = n_ctx - 1 - n_ctx/2;
    const size_t per_pos = size_t(nv)*2;
    std::vector<uint16_t> raw(per_pos/2);

    ear.n_vocab = n_vocab;
    ear.n_pos_per_chunk = n_pos;
    ear.n_chunk = n_chunk;
    ear.n_pos_total = n_chunk * n_pos;
    ear.topk.resize(size_t(ear.n_pos_total) * 10);
    ear.ref_bp_topk.resize(size_t(ear.n_pos_total) * 10);
    ear.ref_p_topk.resize(size_t(ear.n_pos_total) * 10);
    ear.scratch.resize(n_vocab);

    std::vector<float> logp(n_vocab);
    std::vector<int32_t> idx(n_vocab);
    for (int p = 0; p < ear.n_pos_total; ++p) {
        f.read((char *) raw.data(), per_pos);
        if ((size_t) f.gcount() != per_pos) {
            LOG_ERR("ear: truncated ref at position %d", p);
            return false;
        }
        const float scale = ((const float *) raw.data())[0];
        const float min_lp = ((const float *) raw.data())[1];
        for (int i = 0; i < n_vocab; ++i) {
            logp[i] = min_lp + (float) raw[4 + i] * scale;
        }
        // log-softmax (same as Python: max, then logsumexp)
        float m = logp[0];
        for (int i = 1; i < n_vocab; ++i) m = std::max(m, logp[i]);
        double se = 0.0;
        for (int i = 0; i < n_vocab; ++i) se += expf(logp[i] - m);
        const float lse = m + log(se);
        for (int i = 0; i < n_vocab; ++i) {
            logp[i] -= lse;                 // bp
            idx[i] = i;
        }
        // partial select: top-10 largest bp (argpartition equivalent)
        std::partial_sort(idx.begin(), idx.begin() + 10, idx.end(),
                [&logp](int a, int b) { return logp[a] > logp[b]; });
        for (int k = 0; k < 10; ++k) {
            const int gi = p*10 + k;
            ear.topk[gi] = idx[k];
            ear.ref_bp_topk[gi] = logp[idx[k]];
            ear.ref_p_topk[gi]  = expf(logp[idx[k]]);
        }
    }
    LOG_INF("ear: loaded ref %s (%d pos x %d vocab)", path.c_str(), ear.n_pos_total, n_vocab);
    return true;
}

// Accumulate EAR/KL for one chunk of candidate log_probs (u16, nv stride).
// pos_base = global position offset of this chunk's first position.
static void ear_accumulate(ear_ctx & ear, const uint16_t * log_probs, int n_pos, int pos_base) {
    const int n_vocab = ear.n_vocab;
    const int nv = 2*((n_vocab + 1)/2) + 4;
    for (int p = 0; p < n_pos; ++p) {
        const int gp = pos_base + p;
        if (gp >= ear.n_pos_total) break;
        const uint16_t * row = log_probs + size_t(p)*nv;
        const float scale  = ((const float *) row)[0];
        const float min_lp = ((const float *) row)[1];
        // decode candidate logp (same as Python load_logits)
        for (int i = 0; i < n_vocab; ++i) {
            ear.scratch[i] = min_lp + (float) row[4 + i] * scale;
        }
        // log-softmax over FULL vocab (Python re-normalizes the quantized values)
        float m = ear.scratch[0];
        for (int i = 1; i < n_vocab; ++i) m = std::max(m, ear.scratch[i]);
        double se = 0.0;
        for (int i = 0; i < n_vocab; ++i) se += expf(ear.scratch[i] - m);
        const float lse = m + log(se);
        // gather q at the precomputed ref top-K indices
        double ear_pos = 0.0, kl_pos = 0.0;
        const float eps = 1e-12f;
        for (int k = 0; k < 10; ++k) {
            const int gi = gp*10 + k;
            const int idx = ear.topk[gi];
            const float bq = ear.scratch[idx] - lse;
            const float p = ear.ref_p_topk[gi];
            const float q = expf(bq);
            ear_pos += std::min(p, q);
            if (p > eps && q > eps) {
                kl_pos += p * (logf(p) - bq);
            }
...
static void ear_step_begin(ear_ctx & ear) {
    ear.n_seen = 0;
    ear.ear_sum = 0.0;
    ear.kl_sum  = 0.0;
}

// Write one JSON line per step: {"id": "...", "ear": ..., "kl_topk": ...}
static void ear_step_end(ear_ctx & ear, const std::string & id) {
    if (ear.n_seen == 0) {
        LOG_ERR("ear: no positions accumulated for step %s", id.c_str());
        return;
    }
    const double ear_val = ear.ear_sum / ear.n_seen;
    const double kl_val  = ear.kl_sum  / ear.n_seen;
    char buf[256];
    snprintf(buf, sizeof(buf), "{\"id\":\"%s\",\"ear\":%.10g,\"kl_topk\":%.10g}\n",
            id.c_str(), ear_val, kl_val);
    ear.out.write(buf, strlen(buf));
    ear.out.flush();
    LOG_INF("ear step %s: ear=%.7g kl_topk=%.7g", id.c_str(), ear_val, kl_val);
}
...
struct quant_cache; // optional on-disk requantize write-back cache (--quant-cache)
...
struct model_state {
    ...
    std::string src_path;
    quant_cache * cache = nullptr;           // set when --quant-cache is active
};
...
// ---------------------------------------------------------------------------
// quant-cache: on-disk replay of requantize_tensor write-back buffers
// ---------------------------------------------------------------------------
//
// requantize_tensor is a pure function of (pristine bytes, target type,
// imatrix row, tensor shape): the final write-back buffer (the exact bytes
// memcpy'd / fp16-converted into e.t->data) can be stored once per
// (tensor, target) and replayed with a memcpy on later steps. The cache is
// keyed by (model identity, tensor name, target type, imatrix identity) so a
// changed F16 source or imatrix file invalidates cleanly.
//
// Value layout (see examples/fake_quant_ear/quant-cache-design.md §2.3):
//   magic "QCE1" | key (u64) | tensor_name_len (u32) | tensor_name |
//   target_type (u32) | nbytes (u64) | ne[0..3] (4×i64) | payload(nbytes)
//
// A hit validates header fields against the live tensor before replaying;
// any mismatch = miss (re-quantize + rewrite). Partially-written files from a
// crash are discarded by the same validation.
struct quant_cache {
    std::string root;                       // --quant-cache DIR
    uint64_t model_id   = 0;                // F16 source identity (path+size hash)
    uint64_t imatrix_id = 0;                // imatrix file identity (path+size+mtime hash); 0 if unused

    // The quantize phase is OpenMP-parallel (apply_recipe). Each tensor maps
    // to its own cache path, so different threads naturally touch different
    // files, but a same-path collision (overlapping recipe entries) would
    // corrupt the .tmp write. Serialize file I/O defensively; the cost is
    // negligible vs the quantize work being replaced.
    std::mutex mu;

    static constexpr uint32_t MAGIC = 0x31454351; // "QCE1" little-endian

    static uint64_t hash_string(const std::string & s) {
        // FNV-1a 64-bit; cheap, not cryptographic (see design doc §2.2)
    ...
    // model / imatrix identity = resolved path + size + mtime (fast stat, no
    // content read: 18.4 GB model and 5.1 MB imatrix would be wasteful to hash).
    static uint64_t file_identity(const std::string & path) {
        std::error_code ec;
        const auto fs = std::filesystem::file_size(path, ec);
        const auto mt = std::filesystem::last_write_time(path, ec);
        const auto mt_epoch = mt.time_since_epoch().count();
        return hash_string(path + "|" + std::to_string(fs) + "|" + std::to_string(mt_epoch));
    }

    std::string key_for(const tensor_state & e, ggml_type target) const {
        return std::to_string(model_id) + "|" + e.name + "|" + ggml_type_name(target) + "|" + std::to_string(imatrix_id);
    }

    std::string path_for(const tensor_state & e, ggml_type target) const {
        // subdir per target type; the driver removes budget dirs as budgets complete
        ...
        return (std::filesystem::path(root) / ggml_type_name(target) / (safe + ".bin")).string();
    }

    // true on hit: payload memcpy'd into e.t->data. Does NOT touch e.cur
    // (caller sets it after the loop, same as a re-quantize would).
    bool get(const tensor_state & e, ggml_type target) {
        const std::string path = path_for(e, target);
        std::vector<uint8_t> file_buf;
        {
            std::lock_guard<std::mutex> lock(mu);
            std::error_code fec;
            const uintmax_t sz = std::filesystem::file_size(path, fec);
            if (fec || sz < 60) { // magic(4)+key(8)+name_len(4)+type(4)+nbytes(8)+ne(32) before payload
                return false;
            }
            ...
            const size_t rd = fread(file_buf.data(), 1, file_buf.size(), f);
            fclose(f);
            if (rd != file_buf.size()) {
                return false;
            }
        }
        const uint8_t * p = file_buf.data();
        uint32_t magic; memcpy(&magic, p, 4); p += 4;
        if (magic != MAGIC) return false;
        uint64_t key_h;   memcpy(&key_h, p, 8); p += 8;
        const std::string key = key_for(e, target);
        if (key_h != hash_string(key)) return false;
        uint32_t name_len; memcpy(&name_len, p, 4); p += 4;
        if (name_len != e.name.size() || p + name_len > file_buf.data() + file_buf.size()) return false;
        if (memcmp(p, e.name.data(), name_len) != 0) return false; p += name_len;
        uint32_t type;    memcpy(&type, p, 4); p += 4;
        if ((ggml_type) type != target) return false;
        uint64_t nbytes;  memcpy(&nbytes, p, 8); p += 8;
        if (nbytes != e.nbytes) return false;
        int64_t ne[4];    memcpy(ne, p, 32); p += 32;
        for (int d = 0; d < GGML_MAX_DIMS; ++d) {
            if (ne[d] != e.ne[d]) return false;
        }
        if (p + nbytes != file_buf.data() + file_buf.size()) return false;
        memcpy(e.t->data, p, nbytes);
        return true;
    }

    void put(const tensor_state & e, ggml_type target) {
        const std::string path = path_for(e, target);
        std::lock_guard<std::mutex> lock(mu);
        std::error_code ec;
        std::filesystem::create_directories(std::filesystem::path(path).parent_path(), ec);
        if (ec) {
            LOG_WRN("%s: cache mkdir failed for %s: %s", __func__, path.c_str(), ec.message().c_str());
            return;
        }
        // write to a temp file then rename: a crash mid-write leaves a
        // partial file that get() discards by validation instead of a torn
        // file at the final path. The tmp name is unique per process so
        // concurrent workers sharing one qcache dir do not collide on the
        // same .tmp path (rename is atomic on both POSIX and NTFS, and the
        // payload is idempotent: same model+imatrix+tensor+type -> same
        // bytes, so last-writer-wins is safe). A concurrent get() either
        // sees the complete file at the final path or a miss (re-quantize).
        const std::string tmp = path + ".tmp." + std::to_string(GETPID());
        FILE * f = fopen(tmp.c_str(), "wb");
        if (!f) {
            LOG_WRN("%s: cannot open cache %s for writing", __func__, tmp.c_str());
            return;
        }
        const uint32_t magic = MAGIC;
        const std::string key = key_for(e, target);
        const uint64_t key_h = hash_string(key);
        const uint32_t name_len = (uint32_t) e.name.size();
        const uint32_t type = (uint32_t) target;
        const uint64_t nbytes = e.nbytes;
        fwrite(&magic, 1, 4, f);
        fwrite(&key_h, 1, 8, f);
        fwrite(&name_len, 1, 4, f);
        fwrite(e.name.data(), 1, name_len, f);
        fwrite(&type, 1, 4, f);
        fwrite(&nbytes, 1, 8, f);
        fwrite(e.ne, 1, 32, f);
        const size_t w = fwrite(e.t->data, 1, nbytes, f);
        fclose(f);
        if (w != nbytes) {
            std::filesystem::remove(tmp, ec);
            LOG_WRN("%s: short write to %s", __func__, tmp.c_str());
            return;
        }
        std::filesystem::rename(tmp, path, ec);
        if (ec) {
            std::filesystem::remove(tmp, ec);
            LOG_WRN("%s: cache rename %s -> %s failed: %s", __func__, tmp.c_str(), path.c_str(), ec.message().c_str());
        }
    }
};
...
static bool requantize_tensor(const tensor_state & e, const model_state & st, ggml_type target, const float * imatrix, std::string & err) {
    // quant-cache hit: replay the final write-back buffer (memcpy only).
    // The cache is keyed by (model, tensor, target, imatrix), so a hit is
    // byte-identical to re-quantizing (determinism proof, design doc §3.1).
    // Checked BEFORE the pristine read: the payload IS the write-back, so a
    // hit needs no source bytes at all. Restore targets (F16/F32/BF16) are
    // never cached (they just memcpy pristine), so skip the lookup for them.
    const bool is_restore = (target == GGML_TYPE_F16 || target == GGML_TYPE_F32 || target == GGML_TYPE_BF16);
    if (!is_restore && st.cache && st.cache->get(e, target)) {
        return true;
    }
    ...
    // ggml has no unweighted path for IQ2_XXS/IQ2_XS/IQ1_S — those quantizers
    // hard-assert on null weights (see ggml_quantize_requires_imatrix and the
    // impl-level GGML_ASSERT(quant_weights) in ggml-quants.c). The imatrix can
    // never cover token_embd (GET_ROWS, not MUL_MAT) or blk.32 (MTP, not in the
    // main graph) by design, so synthesize unit weights for those targets: this
    // is the "quantizing unweighted" behavior the lookup warning already
    // promises, and it keeps the φ-table consistent across all permutations.
    std::vector<float> unit_imatrix;
    if (imatrix == nullptr && ggml_quantize_requires_imatrix(target)) {
        unit_imatrix.assign((size_t) n_planes * n_per_row, 1.0f);
        imatrix = unit_imatrix.data();
    }
    ...
    if (st.cache) {
        st.cache->put(e, target); // store the write-back buffer for replay
    }
    return true;
}
...
struct plan_step {
    ...
    std::string sw_class;              // class-token switch (sw_is_tensor == false)
    std::string sw_tensor;             // regex switch (sw_is_tensor == true)
    std::regex  sw_re;
...
static std::vector<plan_step> parse_plan_file(const std::string & path) {
    ...
    for (const auto & s : j) {
        ...
        if (s.contains("switch")) {
            ...
            ps.is_switch = true;
            if (sw.contains("tensor")) {
                ps.sw_tensor = sw.at("tensor").get<std::string>();
                try {
                    ps.sw_re = std::regex(ps.sw_tensor);
                } catch (const std::regex_error & re) {
                    throw std::runtime_error("invalid switch regex '" + ps.sw_tensor + "': " + re.what());
                }
            } else if (sw.contains("class")) {
                ...
                throw std::runtime_error("switch must have 'class' or 'tensor'");
            }
...
// relative step: switch one group (class token or per-tensor regex) from its
// current type to another, validating that the walk state matches "from".
static bool apply_switch(const plan_step & ps, model_state & st, const imatrix_data & imx) {
    std::vector<size_t> idxs;
    if (!ps.sw_tensor.empty()) {
        for (size_t i = 0; i < st.tensors.size(); ++i) {
            if (std::regex_search(st.tensors[i].name, ps.sw_re)) {
                idxs.push_back(i);
            }
        }
        if (idxs.empty()) {
            LOG_ERR("%s: switch: regex '%s' matched no tensors (step %s)",
                    __func__, ps.sw_tensor.c_str(), ps.id.c_str());
            return false;
        }
    } else {
...
static bool run_forward(llama_context * ctx, const common_params & params, const int32_t n_ctx,
        std::vector<llama_token> & tokens, const std::string & logits_path, ear_ctx * ear = nullptr) {
    ...
    if (!logits_path.empty()) {   // empty path = --no-logits (EAR-only run)
    ...
    if (ear && ear->enabled && ear->n_chunk != n_chunk) {
        LOG_ERR("%s: EAR ref has %d chunks but this run has %d (EAR positions would misalign)",
                __func__, ear->n_chunk, n_chunk);
        return false;
    }
    const int n_batch = params.n_batch;

    const int n_vocab = llama_vocab_n_tokens(vocab);
    if (ear && ear->enabled && ear->n_vocab != n_vocab) {
        LOG_ERR("%s: EAR ref vocab %d != model vocab %d", __func__, ear->n_vocab, n_vocab);
        return false;
    }
    ...
    for (int i = 0; i < n_chunk; i += n_seq) {
        ...
        for (int seq = 0; seq < n_seq_batch; seq++) {
            ...
            if (ear && ear->enabled) {
                // chunk index for EAR = (i + seq); positions within chunk = n_ctx-1-first
                ear_accumulate(*ear, log_probs.data(), n_ctx - 1 - first, (i + seq) * (n_ctx - 1 - first));
            }
...
struct custom_cli {
    ...
    std::string ear_ref;     // F16 reference logits file (--ear-ref)
    std::string ear_out;     // per-step EAR/KL JSON lines (--ear-out)
    bool no_logits   = false; // skip writing per-step logits files (--no-logits)
    bool keep_f16_copy = false;
    bool allow_fa      = false;  // opt-in: keep CUDA backend (flash-attn on) for speed; candidates within ~1.3e-4 EAR of -dev none
    std::string dump_requant;    // debug: "tensor=TYPE:outfile" — requantize one tensor, dump write-back bytes (determinism proof / cache reference writer)
    std::string quant_cache;     // optional cache dir for requantize write-back buffers (--quant-cache)
};
...
static void usage(const char * exe) {
    fprintf(stderr,
        ...
        "    [--dump-requant <tensor>=<TYPE>:<out>]  # debug: requantize ONE tensor to TYPE, dump the final write-back bytes (determinism proof)\n"
        "    [--quant-cache <dir>]              # cache requantize write-back buffers on disk (per tensor/target; 9x phi-table speedup)\n",
        "    [--ear-ref <f16.logits>]           # F16 reference logits; enables in-tool EAR/KL per step\n",
        "    [--ear-out <ear.jsonl>]            # per-step EAR/KL JSON lines (one per step id)\n",
        "    [--no-logits]                      # skip writing per-step logits files (EAR-only; requires --ear-ref/--ear-out)\n",
        exe);
}
...
static void extract_custom_args(int argc, char ** argv, custom_cli & cli, std::vector<char *> & rest) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--recipe") {
            ...
            cli.allow_fa = true;
        } else if (a == "--dump-requant") {
            if (i + 1 >= argc) throw std::runtime_error("--dump-requant requires a value");
            cli.dump_requant = argv[++i];
        } else if (a == "--quant-cache") {
            if (i + 1 >= argc) throw std::runtime_error("--quant-cache requires a value");
            cli.quant_cache = argv[++i];
        } else if (a == "--ear-ref") {
            if (i + 1 >= argc) throw std::runtime_error("--ear-ref requires a value");
            cli.ear_ref = argv[++i];
        } else if (a == "--ear-out") {
            if (i + 1 >= argc) throw std::runtime_error("--ear-out requires a value");
            cli.ear_out = argv[++i];
        } else if (a == "--no-logits") {
            cli.no_logits = true;
        } else {
...
int llama_fake_quant_ear(int argc, char ** argv) {
    ...
    // The SLQ plans quantize the MTP block (blk.32.*) along with the trunk, so
    // the loader must keep those tensors. Upstream #26296 makes MTP tensors
    // TENSOR_SKIP unless a draft-MTP speculative type is requested; without
    // this, apply_switch fails with "regex '^blk\\.32\\.attn_v\\.weight$'
    // matched no tensors" (observed after the 2026-08-16 buun master merge).
    // There is no draft to run — this only affects which tensors are loaded.
    ...
    try {
        ...
        const bool dump   = !cli.dump_requant.empty();
        if ((single ? 1 : 0) + (plan ? 1 : 0) + (dump ? 1 : 0) != 1) {
            throw std::runtime_error("exactly one of --recipe, --plan or --dump-requant is required");
        }
        if (dump) {
            if (!cli.logits_file.empty()) {
                throw std::runtime_error("--logits-file is not used in --dump-requant mode");
            }
            if (!cli.outdir.empty()) {
                throw std::runtime_error("--outdir is not used in --dump-requant mode");
            }
        }
        ...
        if (!dump && params.prompt.empty()) {
            throw std::runtime_error("no calibration file specified (-f)");
        }

        // ---- dump-requant spec: tensor=TYPE:outfile (parsed once, reused) ----
        std::string dump_tensor, dump_type_str, dump_out;
        ggml_type dump_target = GGML_TYPE_COUNT;
        if (dump) {
            const size_t eq = cli.dump_requant.find('=');
            if (eq == std::string::npos) {
                throw std::runtime_error("--dump-requant expects tensor=TYPE:outfile");
            }
            dump_tensor = cli.dump_requant.substr(0, eq);
            const std::string rest = cli.dump_requant.substr(eq + 1);
            const size_t colon = rest.find(':');
            if (colon == std::string::npos) {
                throw std::runtime_error("--dump-requant expects tensor=TYPE:outfile");
            }
            dump_type_str = rest.substr(0, colon);
            dump_out      = rest.substr(colon + 1);
            dump_target   = parse_type_or_throw(dump_type_str);
            if (dump_out.empty()) {
                throw std::runtime_error("--dump-requant output path is empty");
            }
        }
        ...
        if (single) {
            ...
            needs_imatrix = target_requires_imatrix(dump_target);
        }
        ...
        // through to the F16 source file. Force owned buffers. (buun/master
        // replaced use_mmap with the llama_load_mode enum 2026-08-16.)
        ...
        // dump mode does not run a forward pass, so no calibration text needed.
        ...
        // ---- quant cache ----------------------------------------------------
        // Recipe/plan modes only: dump-requant is the cache's reference writer
        // and must always produce fresh bytes (determinism proof).
        quant_cache qc;
        if (!cli.quant_cache.empty() && !dump) {
            qc.root       = cli.quant_cache;
            qc.model_id   = quant_cache::file_identity(params.model.path);
            qc.imatrix_id = cli.imatrix_path.empty() ? 0 : quant_cache::file_identity(cli.imatrix_path);
            st.cache      = &qc;
            LOG_INF("%s: quant-cache ON at %s (model_id=%016llx imatrix_id=%016llx)",
                    __func__, cli.quant_cache.c_str(),
                    (unsigned long long) qc.model_id, (unsigned long long) qc.imatrix_id);
        } else if (!cli.quant_cache.empty()) {
            LOG_INF("%s: --quant-cache ignored in --dump-requant mode (reference writer must be fresh)", __func__);
        }
        ...
        // ---- in-tool EAR state (--ear-ref/--ear-out) ------------------------
        ear_ctx ear;
        if (!cli.ear_ref.empty() || !cli.ear_out.empty()) {
            if (cli.ear_ref.empty() || cli.ear_out.empty()) {
                throw std::runtime_error("--ear-ref and --ear-out must be used together");
            }
            if (!ear_load_ref(ear, cli.ear_ref)) {
                return 1;
            }
            ear.out.open(cli.ear_out.c_str(), std::ios::out | std::ios::trunc);
            if (!ear.out.is_open()) {
                throw std::runtime_error("cannot open --ear-out: " + cli.ear_out);
            }
            ear.enabled = true;
            LOG_INF("%s: in-tool EAR enabled (ref=%s out=%s)", __func__, cli.ear_ref.c_str(), cli.ear_out.c_str());
        }
        if (cli.no_logits && !ear.enabled) {
            throw std::runtime_error("--no-logits requires --ear-ref/--ear-out (nothing would be written)");
        }

        // ---- run -------------------------------------------------------------
        if (dump) {
            // debug: requantize ONE tensor and dump the final write-back bytes.
            // The write-back is exactly what requantize_tensor memcpy's into
            // e.t->data — the quant-cache reference writer (see
            // examples/fake_quant_ear/quant-cache-design.md section 3.1).
            tensor_state * found = nullptr;
            for (auto & e : st.tensors) {
                if (e.name == dump_tensor) {
                    found = &e;
                    break;
                }
            }
            if (found == nullptr) {
                throw std::runtime_error("tensor not found: " + dump_tensor);
            }
            const float * im = imx.lookup(found->name, found->ne[0] * found->ne[2]);
            std::string err;
            if (!requantize_tensor(*found, st, dump_target, im, err)) {
                throw std::runtime_error(err);
            }
            FILE * f = fopen(dump_out.c_str(), "wb");
            if (!f) {
                throw std::runtime_error("cannot open output file: " + dump_out);
            }
            const size_t written = fwrite(found->t->data, 1, found->nbytes, f);
            fclose(f);
            if (written != found->nbytes) {
                throw std::runtime_error("short write to " + dump_out);
            }
            LOG_INF("dump-requant %s -> %s (%zu bytes, target %s)", dump_tensor.c_str(), dump_out.c_str(), written, ggml_type_name(dump_target));
        } else if (single) {
            ...
            for (const auto & ps : plan_steps) {
                ...
                const std::string out = cli.no_logits ? "" : (std::filesystem::path(cli.outdir) / ps.logits).string();
                if (!run_forward(ctx, params, n_ctx, tokens, out, &ear)) {
                    return 1;
                }
                if (ear.enabled) {
                    ear_step_end(ear, ps.id);
                }
                LOG_INF("step %s done%s%s", ps.id.c_str(), cli.no_logits ? " (no-logits)" : ": " + out, "");
            }
            LOG_INF("\nplan complete: %zu steps -> %s", plan_steps.size(), cli.no_logits ? "(ear only)" : cli.outdir.c_str());
        }
