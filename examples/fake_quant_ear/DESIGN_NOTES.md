...
- **Imatrix size mismatch** — `llama-quantize` *throws* for a non-token_embd
  ...
  missing entry.
- **Unit-imatrix synthesis for imatrix-required targets** — `IQ2_XXS`,
  `IQ2_XS`, `IQ1_S` hard-assert on non-null weights (see
  `ggml_quantize_requires_imatrix` and the impl-level
  `GGML_ASSERT(quant_weights)` in `ggml-quants.c`). Two tensor classes can
  never be covered by an imatrix by design: `token_embd` (GET_ROWS, not
  MUL_MAT) and `blk.32` (MTP, not in the main graph). When a recipe targets
  one of these with an imatrix-required type, the tool synthesizes unit
  weights (`n_planes * n_per_row` of 1.0f) instead of crashing — the
  "quantizing unweighted" behavior the mismatch warning already promises.
  This keeps the φ-table consistent across all permutations (fixed seed,
  identical unit-vector synthesis every run).
...
**2026-08-13 paper re-verify: see `_patches/slq-paper-walkthrough-verified.md`.**
**Changes: (a) EAR driver now records BOTH EAR and top-K KL per step
(paper §3.3 "both computed simultaneously at no additional cost" — TL
Algorithm-2 needs φ^KL); (b) ear_aggregate now reports paper's top-K=10 KL
as `kl_topk` (legacy full-vocab kept as `kl`); (c) anchors backfilled with
kl_topk; (d) TL search will use measured KL over the discrete 12-rung ladder
instead of predicted φ^KL (12 forward passes, minutes).**
