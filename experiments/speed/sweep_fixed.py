...
CSV_PATH = os.path.join(RESULTS_DIR, "dflash_nmax_sweep_fixed.csv")
LOG_PATH = os.path.join(LOG_DIR, "dflash_nmax_sweep_fixed.log")
STATE_PATH = os.path.join(LOG_DIR, "sweep_fixed_state.txt")
...
# All configs: n_min=1..16, n_max=n_min..16
...
for idx, (n_min, n_max) in enumerate(remaining):
    ...
    try:
        ...
        if "error" in data:
            log(f"[{len(done)+idx+1}/{len(all_configs)}] n_min={n_min}, n_max={n_max}: SERVER ERROR - {str(data['error'])[:60]}")
            continue
        ...
        log(f"[{len(done)+idx+1}/{len(all_configs)}] n_min={n_min}, n_max={n_max}: {ct}tok @ {tok_s:.1f}tok/s (ETA {eta/60:.0f}m)")
    except Exception as e:
        log(f"[{len(done)+idx+1}/{len(all_configs)}] n_min={n_min}, n_max={n_max}: ERROR {str(e)[:80]}")
